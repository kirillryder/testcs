...
namespace Feature
{
	...
	const char* SpotList[ ] =
	{
		...
		XorStr( "Neck" ),
		XorStr( "Head" ),
		XorStr( "Left Upper Arm" ),
		XorStr( "Left Forearm" ),
		XorStr( "Left Hand" ),
		XorStr( "Right Upper Arm" ),
	...
	const char* YawList[ ] =
	{
		...
		XorStr( "Fake Forward" ),
		XorStr( "Sideway Left" ),
		XorStr( "Sideway Right" ),
		XorStr( "Fake Sideway Left" ),
		XorStr( "Fake Sideway Right" ),
		XorStr( "Slow Spin" ),
		XorStr( "Fast Spin" ),
		XorStr( "Jitter" ),
		XorStr( "Fake Spin" ),
	...
	void Menu::OnPresentDevice()
	{
		...
		if( m_bTriggerbot )
		{
			ImGui::Begin( XorStr( "Triggerbot" ), nullptr, Flags );
			{
				...
				if( Config::Main->AimbotWeaponConfig )
				{
					...
					ImGui::Selectable( XorStr( "Stomach" ), &Config::Weapon[ i ]->Triggerbot->Stomach );
					...
					ImGui::Checkbox( XorStr( "Auto Wall" ), &Config::Weapon[ i ]->Triggerbot->AutoWall );
					if( Config::Weapon[ i ]->Triggerbot->AutoWall )
						ImGui::SliderInt( XorStr( "Min Damage" ), &Config::Weapon[ i ]->Triggerbot->MinDamage, 0, 100 );

					ImGui::Separator();

					ImGui::Combo( XorStr( "Target" ), &Config::Weapon[ i ]->Triggerbot->Target, TargetList, ARRAYSIZE( TargetList ) );
				...
				else
				{
					...
					ImGui::Selectable( XorStr( "Stomach" ), &Config::Main->Triggerbot->Stomach );
					...
					ImGui::Checkbox( XorStr( "Auto Wall" ), &Config::Main->Triggerbot->AutoWall );
					if( Config::Main->Triggerbot->AutoWall )
						ImGui::SliderInt( XorStr( "Min Damage" ), &Config::Main->Triggerbot->MinDamage, 0, 100 );

					ImGui::Separator();

					ImGui::Combo( XorStr( "Target" ), &Config::Main->Triggerbot->Target, TargetList, ARRAYSIZE( TargetList ) );
		...
		if( m_bConfig )
		{
			ImGui::Begin( XorStr( "Config" ), nullptr, Flags );
			{
				...
				{
					ImGui::Text( XorStr( "You are going to delete \"%s\"!\nAre you sure?\n\n" ), m_szConfigName );
					ImGui::Separator();
	...
	{
		ImGui::Columns( 6, XorStr( "players_columns" ) );
		ImGui::Separator();
