...
void Heart( HMODULE hMod )
{
	...
	while( !Shared::m_bEject )
	{
		if( Shared::m_bLoad )
		{
			...
			ImGui::LoadIniSettingsFromDisk( std::string( Config::GetPath() + XorStr( "gui.ini" ) ).c_str() );
		...
		if( Shared::m_bSave )
		{
			...
			ImGui::SaveIniSettingsToDisk( std::string( Config::GetPath() + XorStr( "gui.ini" ) ).c_str() );
